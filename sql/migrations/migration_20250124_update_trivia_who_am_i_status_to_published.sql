-- Migration: Update trivia_who_am_i status to published
-- Date: 2025-01-24
-- Description: 
--   Updates all trivia_who_am_i records that are not already published or archived
  ...
  IF records_found > 0 THEN
    ...
    FOR record_list IN
      SELECT 
        ...
        LEFT(correct_answer, 40) as answer_preview,
        status,
        published_at,
        created_at
      FROM public.trivia_who_am_i
      ...
      LIMIT 10
    LOOP
      RAISE NOTICE '  ID: %, Preview: "%, Answer: "%, Status: %, Published At: %', 
        record_list.id,
        record_list.question_preview,
        record_list.answer_preview,
...
-- ========================================================================
-- 
-- This script updates all trivia_who_am_i records that are not already
...
-- - A sample list of records that will be updated (with question and answer previews)
