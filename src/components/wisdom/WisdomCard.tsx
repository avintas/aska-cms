...
export default function WisdomCard({
  ...
  const handleStatusChange = async (newStatus: string): Promise<void> => {
    ...
    try {
      const response = await fetch(`/api/wisdom/${wisdom.id}`, {
        method: 'PATCH',
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Failed to update wisdom status';
      alert(`Failed to update wisdom status: ${errorMessage}`);
    }
  };
