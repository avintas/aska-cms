...
import {
  generateWisdomAction,
  analyzeSourceContent,
  getGeneratedWisdomItems,
  searchSourcesForWisdom,
  type WisdomPrompt,
  type ContentAnalysis,
  type GeneratedWisdomItem,
} from '@/app/w-gen/actions';
import type { IdeationSourceSummary } from '@/lib/ideation';

interface WGenWorkspaceProps {
  prompt: WisdomPrompt | null;
  initialSources: IdeationSourceSummary[];
  initialTotal: number;
  initialError: string | null;
}

export default function WGenWorkspace({
  prompt,
  initialSources,
  initialTotal,
  initialError,
}: WGenWorkspaceProps): JSX.Element {
  ...
  const [systemMessages, setSystemMessages] = useState<string[]>(() => {
    ...
    if (!prompt) {
      return ['⚠️ No Penalty Box Philosopher prompt found. Please ensure prompt exists in collection_prompts.'];
    }
    return ['✨ Ready to generate wisdom content!'];
  });
  ...
  const [generatedItems, setGeneratedItems] = useState<GeneratedWisdomItem[]>([]);
  ...
  const fetchSources = useCallback(async (page: number) => {
    ...
    try {
      const result = await searchSourcesForWisdom({ page, pageSize });
  ...
  useEffect(() => {
    if (currentSourceId) {
      ...
      if (currentSourceId !== justGeneratedSourceId) {
        ...
        getGeneratedWisdomItems(currentSourceId).then((result) => {
  ...
  const handleGenerate = (): void => {
    ...
    if (!prompt) {
      setSystemMessages((prev) => [...prev, '❌ No Penalty Box Philosopher prompt available. Cannot generate.']);
      return;
    }

    setSystemMessages((prev) => [
      ...prev,
      `🚀 Starting wisdom generation with source: "${selectedSource?.title || `ID ${currentSourceId}`}"...`,
    ]);

    startTransition(() => {
      generateWisdomAction(currentSourceId).then((result) => {
        setLastResult(result);
        if (result.success) {
          setSystemMessages((prev) => [
            ...prev,
            `✅ ${result.message}`,
            `📊 Generated ${result.itemCount || 0} wisdom item(s)`,
          ]);
          ...
          if (result.generatedItems && result.generatedItems.length > 0) {
            ...
            getGeneratedWisdomItems(currentSourceId).then((itemsResult) => {
  ...
  return (
    <div className="space-y-10">
      ...
      <SectionCard
        eyebrow="THE BOX"
        title="W-Gen"
        description="The Soul. Witty, cynical, '2-minute' wisdom. Generate philosophical insights from the penalty box."
        action={
                        ...
                        <div className="space-y-4">
                          <div className="rounded-xl border-2 border-slate-200 bg-gradient-to-br from-slate-50 to-white p-4 dark:border-slate-700 dark:from-slate-800 dark:to-slate-900">
                            ...
                            <div className="grid grid-cols-2 gap-3 text-xs">
                              ...
                              <div className="rounded-lg bg-white p-2 dark:bg-slate-900">
                                <span className="text-slate-500 dark:text-slate-400">Quotes:</span>{' '}
                                <span className="font-bold text-slate-900 dark:text-slate-100">{contentAnalysis.quoteCount}</span>
                              </div>
                              <div className="rounded-lg bg-white p-2 dark:bg-slate-900">
              ...
              <div className="flex items-center justify-between">
                ...
                {generatedItems.length > 0 && (
                  <span className="rounded-full bg-primary-brand/20 px-4 py-1.5 text-sm font-bold text-primary-brand">
                    {generatedItems.length} {generatedItems.length === 1 ? 'item' : 'items'}
                  </span>
                )}
              </div>
              {isLoadingItems ? (
                ...
                <div className="rounded-xl border-2 border-dashed border-slate-300 bg-slate-50 p-12 text-center dark:border-slate-700 dark:bg-slate-800">
                  <div className="text-4xl mb-3">🎯</div>
                  <p className="text-sm font-medium text-slate-600 dark:text-slate-400">
                    No wisdom generated yet. Click "Generate Wisdom Content" to create some!
                  </p>
                </div>
              ) : (
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">
                The Soul. Witty, cynical, '2-minute' wisdom.
              </p>
            </div>
          </div>
