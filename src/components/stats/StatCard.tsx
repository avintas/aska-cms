...
export default function StatCard({
  ...
  const wordCount = countWords(stat.stat_text);
  const charCount = countCharacters(stat.stat_text);

  // Copy stat text to clipboard
  const handleCopy = async (): Promise<void> => {
    try {
      let textToCopy = stat.stat_text;
      if (stat.stat_value) {
        textToCopy = `${textToCopy} (${stat.stat_value})`;
      }
      if (stat.attribution) {
        textToCopy = `${textToCopy} — ${stat.attribution}`;
      }
      await navigator.clipboard.writeText(textToCopy);
    } catch (error) {
      // eslint-disable-next-line no-console
      console.error('Failed to copy:', error);
      alert('Failed to copy stat text');
    }
  };
  ...
  const handleStatusChange = async (newStatus: string): Promise<void> => {
    ...
    try {
      ...
      onStatusChange(stat.id, newStatus);
    } catch (error) {
      // eslint-disable-next-line no-console
      console.error('Failed to update status:', error);
      const errorMessage =
        error instanceof Error ? error.message : 'Failed to update stat status';
      alert(`Failed to update stat status: ${errorMessage}`);
    }
  };
  ...
  // Determine status
  const status = stat.status || 'unpublished';
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm transition-shadow hover:shadow-md dark:border-slate-800 dark:bg-slate-900/70">
      ...
      <div className="mb-2 flex items-center gap-2">
        ...
        <div className="text-xs text-gray-500 dark:text-gray-400">ID: {stat.id}</div>
      </div>
      ...
      {/* Stat Value and Metadata */}
      <div className="mb-2 flex flex-wrap items-center gap-1.5 text-xs text-gray-600 dark:text-gray-400">
        {stat.stat_value && <span>Value: {stat.stat_value}</span>}
        {stat.year && (
          <>
            {stat.stat_value && <span>•</span>}
            <span>Year: {stat.year}</span>
          </>
        )}
        {stat.stat_category && (
          <>
            {(stat.stat_value || stat.year) && <span>•</span>}
            <span>Category: {stat.stat_category}</span>
          </>
        )}
        {stat.attribution && (
          <>
            {(stat.stat_value || stat.year || stat.stat_category) && <span>•</span>}
            <span className="italic">— {stat.attribution}</span>
          </>
        )}
      </div>
