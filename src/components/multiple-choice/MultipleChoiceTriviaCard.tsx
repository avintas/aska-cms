...
export default function MultipleChoiceTriviaCard({
  ...
  onStatusChange,
  onDelete,
}: MultipleChoiceTriviaCardProps): JSX.Element {
  const questionWordCount = countWords(trivia.question_text);
  const questionCharCount = countCharacters(trivia.question_text);
  const answerWordCount = countWords(trivia.correct_answer);
  const answerCharCount = countCharacters(trivia.correct_answer);
  ...
  const wrongAnswersCharCount = trivia.wrong_answers
    ? trivia.wrong_answers.reduce((sum, ans) => sum + countCharacters(ans), 0)
    : 0;
  const wrongAnswersWordCount = trivia.wrong_answers
    ? trivia.wrong_answers.reduce((sum, ans) => sum + countWords(ans), 0)
    : 0;

  const totalCharCount =
    questionCharCount + answerCharCount + explanationCharCount + wrongAnswersCharCount;
  const totalWordCount =
    questionWordCount + answerWordCount + explanationWordCount + wrongAnswersWordCount;
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm transition-shadow hover:shadow-md dark:border-slate-800 dark:bg-slate-900/70">
      ...
      {/* Correct Answer */}
      <div className="mb-2 rounded bg-green-50 p-2 dark:bg-green-950/30">
        <p className="text-xs font-medium text-green-900 dark:text-green-200">Correct Answer:</p>
        <p className="text-sm text-green-800 dark:text-green-100">{trivia.correct_answer}</p>
      </div>

      {/* Wrong Answers */}
      {trivia.wrong_answers && trivia.wrong_answers.length > 0 && (
        <div className="mb-2 rounded bg-red-50 p-2 dark:bg-red-950/30">
          <p className="text-xs font-medium text-red-900 dark:text-red-200">Wrong Answers:</p>
          <ul className="mt-1 space-y-1">
            {trivia.wrong_answers.map((answer, index) => (
              <li key={index} className="text-sm text-red-800 dark:text-red-100">
                {index + 1}. {answer}
              </li>
            ))}
          </ul>
