...
import type { Motivational } from '@aska/shared';
import { StatusBadge } from '@/components/ui/CollectionList';

interface CaptainHeartCardProps {
...
export default function CaptainHeartCard({
  motivational,
  onStatusChange,
  onDelete,
}: CaptainHeartCardProps): JSX.Element {
  const quoteWordCount = countWords(motivational.quote);
  const quoteCharCount = countCharacters(motivational.quote);
  const totalCharCount =
    quoteCharCount +
    (motivational.author ? countCharacters(motivational.author) : 0) +
    (motivational.context ? countCharacters(motivational.context) : 0);

  const handleCopy = async (): Promise<void> => {
    try {
      let textToCopy = `"${motivational.quote}"`;
      if (motivational.author) {
        textToCopy = `${textToCopy}\n— ${motivational.author}`;
      }
      if (motivational.context) {
  ...
  const handleStatusChange = async (newStatus: string): Promise<void> => {
    ...
    try {
      const response = await fetch(`/api/captain-heart/${motivational.id}`, {
        method: 'PATCH',
      ...
      onStatusChange(motivational.id, newStatus);
    } catch (error) {
      // eslint-disable-next-line no-console
      console.error('Failed to update status:', error);
      const errorMessage =
        error instanceof Error ? error.message : 'Failed to update captain heart status';
      alert(`Failed to update captain heart status: ${errorMessage}`);
    }
  };
  ...
  const handleDelete = async (): Promise<void> => {
    ...
    try {
      ...
      onDelete(motivational.id);
    } catch (error) {
      // eslint-disable-next-line no-console
      console.error('Failed to delete:', error);
      alert('Failed to delete captain heart entry');
    }
  };

  const status = motivational.status || 'unpublished';
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm transition-shadow hover:shadow-md dark:border-slate-800 dark:bg-slate-900/70">
      <div className="mb-2 flex items-center gap-2">
        ...
        <div className="text-xs text-gray-500 dark:text-gray-400">ID: {motivational.id}</div>
      </div>

      <p className="mb-2 text-sm text-gray-900 leading-relaxed dark:text-gray-100">
        &ldquo;{motivational.quote}&rdquo;
      </p>
      ...
      {motivational.context && (
        <p className="mb-2 text-xs text-gray-600 dark:text-gray-400">{motivational.context}</p>
      )}
