...
export default function FactCard({
  ...
  const wordCount = countWords(fact.fact_text);
  const charCount = countCharacters(fact.fact_text);

  // Copy fact text to clipboard
  const handleCopy = async (): Promise<void> => {
    try {
      let textToCopy = fact.fact_text;
      if (fact.fact_value) {
        textToCopy = `${textToCopy} (${fact.fact_value})`;
      }
      if (fact.attribution) {
        textToCopy = `${textToCopy} — ${fact.attribution}`;
      }
      await navigator.clipboard.writeText(textToCopy);
    } catch (error) {
      // eslint-disable-next-line no-console
      console.error('Failed to copy:', error);
      alert('Failed to copy fact text');
    }
  };
  ...
  const handleStatusChange = async (newStatus: string): Promise<void> => {
    ...
    try {
      ...
      onStatusChange(fact.id, newStatus);
    } catch (error) {
      // eslint-disable-next-line no-console
      console.error('Failed to update status:', error);
      const errorMessage =
        error instanceof Error ? error.message : 'Failed to update fact status';
      alert(`Failed to update fact status: ${errorMessage}`);
    }
  };
  ...
  // Determine status
  const status = fact.status || 'unpublished';
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm transition-shadow hover:shadow-md dark:border-slate-800 dark:bg-slate-900/70">
      ...
      <div className="mb-2 flex items-center gap-2">
        ...
        <div className="text-xs text-gray-500 dark:text-gray-400">ID: {fact.id}</div>
      </div>
      ...
      {/* Fact Value and Metadata */}
      <div className="mb-2 flex flex-wrap items-center gap-1.5 text-xs text-gray-600 dark:text-gray-400">
        {fact.fact_value && <span>Value: {fact.fact_value}</span>}
        {fact.year && (
          <>
            {fact.fact_value && <span>•</span>}
            <span>Year: {fact.year}</span>
          </>
        )}
        {fact.fact_category && (
          <>
            {(fact.fact_value || fact.year) && <span>•</span>}
            <span>Category: {fact.fact_category}</span>
          </>
        )}
        {fact.attribution && (
          <>
            {(fact.fact_value || fact.year || fact.fact_category) && <span>•</span>}
            <span className="italic">— {fact.attribution}</span>
          </>
        )}
      </div>
