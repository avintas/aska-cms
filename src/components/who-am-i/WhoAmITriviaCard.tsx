...
export default function WhoAmITriviaCard({
  ...
  onStatusChange,
  onDelete,
}: WhoAmITriviaCardProps): JSX.Element {
  ...
  const totalCharCount = questionCharCount + answerCharCount + explanationCharCount;
  const totalWordCount = questionWordCount + answerWordCount + explanationWordCount;
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm transition-shadow hover:shadow-md dark:border-slate-800 dark:bg-slate-900/70">
      ...
      {/* Correct Answer */}
      <div className="mb-2 rounded bg-blue-50 p-2 dark:bg-blue-950/30">
        <p className="text-xs font-medium text-blue-900 dark:text-blue-200">Answer:</p>
        <p className="text-sm text-blue-800 dark:text-blue-100">{trivia.correct_answer}</p>
      </div>
