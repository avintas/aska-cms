...
import {
  generateCaptainHeartAction,
  analyzeSourceContent,
  getGeneratedCaptainHeartItems,
  searchSourcesForCaptainHeart,
  type CaptainHeartPrompt,
  type ContentAnalysis,
  type GeneratedMotivateItem,
} from '@/app/captain-heart-gen/actions';
import type { IdeationSourceSummary } from '@/lib/ideation';

interface CaptainHeartGenWorkspaceProps {
  prompt: CaptainHeartPrompt | null;
  initialSources: IdeationSourceSummary[];
  initialTotal: number;
  initialError: string | null;
}

export default function CaptainHeartGenWorkspace({
  prompt,
  initialSources,
  initialTotal,
  initialError,
}: CaptainHeartGenWorkspaceProps): JSX.Element {
  ...
  const [systemMessages, setSystemMessages] = useState<string[]>(() => {
    ...
    if (!prompt) {
      return ['⚠️ No Captain Heart prompt found. Please ensure Captain Heart prompt exists in collection_prompts.'];
    }
    return ['✨ Ready to generate Captain Heart messages!'];
  });
  ...
  const fetchSources = useCallback(async (page: number) => {
    ...
    try {
      const result = await searchSourcesForCaptainHeart({ page, pageSize });
  ...
  useEffect(() => {
    if (currentSourceId) {
      ...
      if (currentSourceId !== justGeneratedSourceId) {
        ...
        getGeneratedCaptainHeartItems(currentSourceId).then((result) => {
  ...
  const handleGenerate = (): void => {
    ...
    if (!prompt) {
      setSystemMessages((prev) => [...prev, '❌ No Captain Heart prompt available. Cannot generate.']);
      return;
    }

    setSystemMessages((prev) => [
      ...prev,
      `🚀 Starting Captain Heart generation with source: "${selectedSource?.title || `ID ${currentSourceId}`}"...`,
    ]);

    startTransition(() => {
      generateCaptainHeartAction(currentSourceId).then((result) => {
        setLastResult(result);
        if (result.success) {
          setSystemMessages((prev) => [
            ...prev,
            `✅ ${result.message}`,
            `📊 Generated ${result.itemCount || 0} message(s)`,
          ]);
          ...
          if (result.generatedItems && result.generatedItems.length > 0) {
            ...
            getGeneratedCaptainHeartItems(currentSourceId).then((itemsResult) => {
  ...
  const getPromptPreview = (prompt: CaptainHeartPrompt | null): string => {
  ...
  return (
    <div className="space-y-10">
      ...
      <SectionCard
        ...
        description="The Heart. Connection, love, and hype. Generate warm, supportive, emotional messages."
        action={
                      ...
                      >
                        {isPending ? (
                          ...
                          <>
                            ...
                            Generate Captain Heart Messages
                          </>
                        )}
                      </PrimaryButton>
                    </div>
              ...
              <div className="flex items-center justify-between">
                ...
                {generatedItems.length > 0 && (
                  <span className="rounded-full bg-primary-brand/20 px-4 py-1.5 text-sm font-bold text-primary-brand">
                    {generatedItems.length} {generatedItems.length === 1 ? 'message' : 'messages'}
                  </span>
                )}
              </div>
              {isLoadingItems ? (
                ...
                <div className="rounded-xl border-2 border-dashed border-slate-300 bg-slate-50 p-12 text-center dark:border-slate-700 dark:bg-slate-800">
                  <div className="text-4xl mb-3">🎯</div>
                  <p className="text-sm font-medium text-slate-600 dark:text-slate-400">
                    No messages generated yet. Click "Generate Captain Heart Messages" to create some!
                  </p>
                </div>
              ) : (
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">
                The Heart. Connection, love, and hype.
              </p>
            </div>
          </div>
