...
export default function BenchBossCard({
  motivational,
  onStatusChange,
  onDelete,
}: BenchBossCardProps): JSX.Element {
  ...
  const handleStatusChange = async (newStatus: string): Promise<void> => {
    ...
    try {
      const response = await fetch(`/api/bench-boss/${motivational.id}`, {
        method: 'PATCH',
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Failed to update bench boss status';
      alert(`Failed to update bench boss status: ${errorMessage}`);
    }
  };
  ...
  const handleDelete = async (): Promise<void> => {
    ...
    try {
      ...
      alert('Failed to delete bench boss entry');
    }
  };
