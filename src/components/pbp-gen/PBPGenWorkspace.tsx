...
import {
  ...
  getGeneratedPBPItems,
  searchSourcesForPBP,
  type PBPPrompt,
  type ContentAnalysis,
  type GeneratedPBPItem,
} from '@/app/pbp-gen/actions';
import type { IdeationSourceSummary } from '@/lib/ideation';

interface PBPGenWorkspaceProps {
  prompt: PBPPrompt | null;
  initialSources: IdeationSourceSummary[];
  initialTotal: number;
  initialError: string | null;
}

export default function PBPGenWorkspace({
  prompt,
  initialSources,
  initialTotal,
  initialError,
}: PBPGenWorkspaceProps): JSX.Element {
  ...
  const [generatedItems, setGeneratedItems] = useState<GeneratedPBPItem[]>([]);
  ...
  const fetchSources = useCallback(async (page: number) => {
    ...
    try {
      const result = await searchSourcesForPBP({ page, pageSize });
  ...
  useEffect(() => {
    if (currentSourceId) {
      ...
      if (currentSourceId !== justGeneratedSourceId) {
        ...
        getGeneratedPBPItems(currentSourceId).then((result) => {
  ...
  const handleGenerate = (): void => {
    ...
    startTransition(() => {
      generatePBPAction(currentSourceId).then((result) => {
        ...
        if (result.success) {
          ...
          if (result.generatedItems && result.generatedItems.length > 0) {
            ...
            getGeneratedPBPItems(currentSourceId).then((itemsResult) => {
