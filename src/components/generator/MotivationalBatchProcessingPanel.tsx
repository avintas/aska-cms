...
import {
  batchGenerateMotivationalAction,
  getUnprocessedSourcesCountForMotivational,
  type BatchProcessResult,
  type UnprocessedSourcesCount,
} from '@/app/main-generator/actions';

export default function MotivationalBatchProcessingPanel(): JSX.Element {
  ...
  useEffect(() => {
    const fetchCount = async (): Promise<void> => {
      ...
      try {
        const countData = await getUnprocessedSourcesCountForMotivational();
  ...
  const handleStartProcessing = (): void => {
    ...
    startTransition(() => {
      batchGenerateMotivationalAction(count).then((batchResult) => {
  ...
  return (
    <SectionCard
      ...
      title="Batch Generate Motivational Messages"
      description="Process multiple source content pieces sequentially to generate motivational messages. Each source is processed one-by-one with a 2-second cooldown between requests."
      collapsible={true}
    >
        ...
        <div className="space-y-4">
          <div>
            <div className="flex items-center justify-between mb-2">
              <label
                htmlFor="process-count-motivational"
            ...
            <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
              ...
              processed for motivational messages.
            </p>
            {sourcesCount !== null && sourcesCount.available === 0 && (
