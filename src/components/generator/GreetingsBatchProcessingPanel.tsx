...
import {
  batchGenerateGreetingsAction,
  getUnprocessedSourcesCountForGreetings,
  type BatchProcessResult,
  type UnprocessedSourcesCount,
} from '@/app/main-generator/actions';

export default function GreetingsBatchProcessingPanel(): JSX.Element {
  ...
  useEffect(() => {
    const fetchCount = async (): Promise<void> => {
      ...
      try {
        const countData = await getUnprocessedSourcesCountForGreetings();
  ...
  const handleStartProcessing = (): void => {
    ...
    startTransition(() => {
      batchGenerateGreetingsAction(count).then((batchResult) => {
  ...
  return (
    <SectionCard
      ...
      title="Batch Generate Greetings"
      description="Process multiple source content pieces sequentially to generate greetings. Each source is processed one-by-one with a 2-second cooldown between requests."
      collapsible={true}
    >
        ...
        <div className="space-y-4">
          <div>
            <div className="flex items-center justify-between mb-2">
              <label
                htmlFor="process-count-greetings"
