...
import {
  batchGenerateFactsAction,
  getUnprocessedSourcesCountForFacts,
  type BatchProcessResult,
  type UnprocessedSourcesCount,
} from '@/app/main-generator/actions';

export default function StatsBatchProcessingPanel(): JSX.Element {
  ...
  useEffect(() => {
    const fetchCount = async (): Promise<void> => {
      ...
      try {
        const countData = await getUnprocessedSourcesCountForFacts();
  ...
  const handleStartProcessing = (): void => {
    ...
    startTransition(() => {
      batchGenerateFactsAction(count).then((batchResult) => {
  ...
  return (
    <SectionCard
      ...
      title="Batch Generate Stats"
      description="Process multiple source content pieces sequentially to generate stats. Each source is processed one-by-one with a 2-second cooldown between requests."
      collapsible={true}
    >
