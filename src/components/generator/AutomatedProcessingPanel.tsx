...
import {
  batchGenerateMultipleChoiceAction,
  getUnprocessedSourcesCount,
  type BatchProcessResult,
  type UnprocessedSourcesCount,
} from '@/app/main-generator/actions';

export default function AutomatedProcessingPanel(): JSX.Element {
  ...
  useEffect(() => {
    const fetchCount = async (): Promise<void> => {
      ...
      try {
        const countData = await getUnprocessedSourcesCount();
  ...
  const handleStartProcessing = (): void => {
    ...
    startTransition(() => {
      batchGenerateMultipleChoiceAction(count).then((batchResult) => {
  ...
  return (
    <SectionCard
      ...
      title="Batch Generate Multiple Choice Trivia"
      description="Process multiple source content pieces sequentially to generate multiple-choice trivia questions. Each source is processed one-by-one with a 2-second cooldown between requests."
      collapsible={true}
    >
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
              ...
              processed for multiple-choice trivia.
            </p>
            {sourcesCount !== null && sourcesCount.available === 0 && (
