...
import {
  batchGenerateWisdomAction,
  getUnprocessedSourcesCountForWisdom,
  type BatchProcessResult,
  type UnprocessedSourcesCount,
} from '@/app/main-generator/actions';

export default function WisdomBatchProcessingPanel(): JSX.Element {
  ...
  useEffect(() => {
    const fetchCount = async (): Promise<void> => {
      ...
      try {
        const countData = await getUnprocessedSourcesCountForWisdom();
  ...
  const handleStartProcessing = (): void => {
    ...
    startTransition(() => {
      batchGenerateWisdomAction(count).then((batchResult) => {
  ...
  return (
    <SectionCard
      ...
      title="Batch Generate Wisdom Messages"
      description="Process multiple source content pieces sequentially to generate wisdom messages. Each source is processed one-by-one with a 2-second cooldown between requests."
      collapsible={true}
    >
