...
export default function BuildTrueFalseSetsClient(): JSX.Element {
  ...
  const handleBuildSets = async (): Promise<void> => {
    ...
    try {
      const response = await fetch('/api/cron/build-automated-sets', {
        ...
        body: JSON.stringify({
          numberOfSets,
          triviaType: 'tf', // Always True/False for this page
  ...
  return (
    <div className="space-y-8">
      <SectionCard
        eyebrow="True/False Sets"
        title="Build True/False Trivia Sets"
        description="Create true/false trivia sets with custom configuration. Each set will be stored as a separate record in collection_trivia_sets."
      >
        <div className="mt-6 space-y-8">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/70">
            ...
            />
            <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
              Number of true/false questions per trivia set.
            </p>
          </div>
