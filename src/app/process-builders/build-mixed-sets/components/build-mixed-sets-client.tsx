...
export default function BuildMixedSetsClient(): JSX.Element {
  ...
  const handleBuildSets = async (): Promise<void> => {
    ...
    try {
      const response = await fetch('/api/cron/build-automated-sets', {
        ...
        body: JSON.stringify({
          numberOfSets,
          triviaType: 'mix', // Mixed sets (alternates between MC and TF)
  ...
  return (
    <div className="space-y-8">
      <SectionCard
        eyebrow="Mixed Sets"
        title="Build Mixed Trivia Sets"
        description="Create mixed trivia sets (alternating between Multiple Choice and True/False) with custom configuration. Each set will be stored as a separate record in collection_trivia_sets."
      >
        <div className="mt-6 space-y-8">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/70">
            ...
            <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
              {numberOfSets === 1
                ? 'Building one set at a time allows for better quality control and review. Sets will alternate between Multiple Choice and True/False types.'
                : 'Sets will alternate between Multiple Choice and True/False types. Each set will be created as a separate record in the database.'}
            </p>
          </div>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/70">
            ...
            />
            <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
              Number of questions per trivia set (will be MC or TF based on set type).
            </p>
          </div>
