...
// GET /api/who-am-i - Fetch who am i trivia entries with filters or stats
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (statsOnly) {
      ...
      const { data: allItems } = await supabase.from('trivia_who_am_i').select('status');
    ...
    if (error) {
      ...
      console.error('Error fetching who am i trivia:', error);
      return NextResponse.json(
        { success: false, error: 'Failed to fetch who am i trivia entries' },
        { status: 500 },
      );
    }
