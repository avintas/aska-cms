...
// GET /api/stats - Fetch stat entries with filters or stats
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (statsOnly) {
      ...
      const { data: allItems } = await supabase.from('collection_stats').select('status');
    ...
    if (error) {
      ...
      return NextResponse.json(
        { success: false, error: 'Failed to fetch stat entries' },
        { status: 500 },
      );
    }
