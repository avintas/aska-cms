...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (statsOnly) {
      ...
      const { data: allItems } = await supabase.from('collection_motivational').select('status');
    ...
    if (error) {
      ...
      console.error('Error fetching motivational:', error);
      return NextResponse.json(
        { success: false, error: 'Failed to fetch motivational entries' },
        { status: 500 },
      );
    }
