...
// GET /api/captain-heart - Fetch Captain Heart entries with filters or stats
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (statsOnly) {
      // Get all items filtered by attribution and count by status
    ...
    if (error) {
      ...
      console.error('Error fetching captain heart:', error);
      return NextResponse.json(
        { success: false, error: 'Failed to fetch captain heart entries' },
        { status: 500 },
      );
    }
