...
// GET /api/bench-boss - Fetch Bench Boss entries with filters or stats
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (error) {
      ...
      console.error('Error fetching bench boss:', error);
      return NextResponse.json(
        { success: false, error: 'Failed to fetch bench boss entries' },
        { status: 500 },
      );
    }
