...
// PATCH /api/facts/[id] - Partially update fact (archive/status changes)
export async function PATCH(
  ...
  try {
    ...
    console.log('Updating fact:', { id, updateData });
    ...
    if (error) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: 'Failed to update fact status',
...
// DELETE /api/facts/[id] - Delete fact entry
export async function DELETE(
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.json(
        { success: false, error: 'Failed to delete fact entry' },
        { status: 500 },
      );
    }
