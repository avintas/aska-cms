...
// GET /api/facts - Fetch fact entries with filters or stats
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (statsOnly) {
      ...
      const { data: allItems } = await supabase.from('collection_facts').select('status');
    ...
    if (error) {
      ...
      return NextResponse.json(
        { success: false, error: 'Failed to fetch fact entries' },
        { status: 500 },
      );
    }
