...
import type { WhoAmITrivia, WhoAmITriviaUpdateInput } from '@aska/shared';

interface PageProps {
  params: Promise<{ id: string }>;
}

async function updateWhoAmITriviaAction(id: number, formData: FormData): Promise<void> {
  'use server';

  const supabase = await createServerClient();

  const triviaData: WhoAmITriviaUpdateInput = {
  ...
  const { error } = await supabase.from('trivia_who_am_i').update(triviaData).eq('id', id);

  if (error) {
    console.error('Error updating who am i trivia:', error);
...
async function deleteWhoAmITriviaAction(id: number): Promise<void> {
  'use server';

  const supabase = await createServerClient();

  const { error } = await supabase.from('trivia_who_am_i').delete().eq('id', id);

  if (error) {
    console.error('Error deleting who am i trivia:', error);
...
const EditWhoAmITriviaPage: FC<PageProps> = async ({ params }) => {
  ...
  const triviaData = trivia as WhoAmITrivia;

  const updateAction = updateWhoAmITriviaAction.bind(null, triviaId);
  const deleteAction = deleteWhoAmITriviaAction.bind(null, triviaId);
            ...
            <div className="px-4 py-5 sm:p-6 space-y-6">
              <div>
                ...
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  Enter progressive clues, one per line
                </p>
                <textarea
                  name="question_text"
                  id="question_text"
...
export default EditWhoAmITriviaPage;
