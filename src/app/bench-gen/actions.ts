...
export interface BenchBossPrompt {
  id: number;
  prompt_name: string;
...
/**
 * Fetch the Bench Boss prompt from collection_prompts table
 */
export async function getBenchBossPrompt(): Promise<{
  success: boolean;
  ...
  try {
    ...
    if (!data) {
      return {
        success: false,
        error: 'No Bench Boss prompt found in collection_prompts table.',
      };
    }
...
/**
 * Search sources for Bench Boss generator with pagination and usage badges
 */
export async function searchSourcesForBenchBoss(
...
/**
 * Check if source has been used for Bench Boss generation
 */
export async function isSourceUsedForBenchBoss(sourceId: number): Promise<boolean> {
  try {
    const supabase = await createServerClient();

    // Check collection_hockey_motivate table - only check for Bench Boss items
    const { data, error } = await supabase
      .from('collection_hockey_motivate')
      .select('id')
      .eq('source_content_id', sourceId)
      .eq('attribution', 'Bench Boss') // Only check Bench Boss directives
      .limit(1)
      .maybeSingle();
...
/**
 * Mark source as used for Bench Boss generation
 */
async function markSourceAsUsedForBenchBoss(sourceId: number): Promise<void> {
  try {
    const supabase = await createServerClient();
 ...
 */
function normalizeBenchBossItem(
  ...
  // Bench Boss prompt uses "content_body", map to quote field
  const quote =
  ...
  // Bench Boss prompt uses "category_tag", map to category field
  const category = coerceString(item.category_tag) ?? coerceString(item.category) ?? null;

  // Bench Boss prompt uses "moment_type", map to theme field
  const theme = coerceString(item.moment_type) ?? coerceString(item.theme) ?? null;

  // Bench Boss prompt uses "character_voice" or "attribution" field
  // If not set, default to "Bench Boss" since this is the Bench Boss generator
  const attribution =
    coerceString(item.character_voice) ??
    coerceString(item.attribution) ??
    coerceString(item.author) ??
    'Bench Boss'; // Default to Bench Boss if not specified
  ...
  try {
    ...
    return {
      ...
      analysis: {
        wordCount,
        sentenceCount,
        quoteCount: 0, // Not applicable for Bench Boss
        wisdomIndicators,
 ...
 */
export async function getGeneratedBenchBossItems(sourceId: number): Promise<{
  success: boolean;
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .eq('source_content_id', sourceId)
      .eq('attribution', 'Bench Boss') // Only show Bench Boss directives
...
/**
 * Generate Bench Boss content and save to collection_hockey_motivate table
 */
export async function generateBenchBossAction(sourceId: number): Promise<GenerationResult> {
  try {
    // Check if source has already been used
    const alreadyUsed = await isSourceUsedForBenchBoss(sourceId);
    if (alreadyUsed) {
      return {
        success: false,
        message: 'This source has already been used for Bench Boss generation. To avoid duplicates, select a different source.',
      };
    }

    // Fetch Bench Boss prompt
    const promptResult = await getBenchBossPrompt();
    if (!promptResult.success || !promptResult.prompt) {
      return {
        success: false,
        message: promptResult.error || 'Failed to load Bench Boss prompt.',
      };
    }
    ...
    if (geminiResult.data.length === 0) {
      return {
        success: false,
        message: 'Gemini did not generate any items. The source content may not be suitable for Bench Boss directive extraction, or the prompt may need adjustment.',
      };
    }

    // Normalize items
    const normalizedItems = geminiResult.data
      .map((item) => normalizeBenchBossItem(item, sourceId))
    ...
    // Mark source as used for Bench Boss generation
    await markSourceAsUsedForBenchBoss(sourceId);

    return {
      success: true,
      message: `✅ Successfully generated and saved ${normalizedItems.length} Bench Boss directive(s)!`,
