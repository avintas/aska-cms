...
import type { MultipleChoiceTriviaSet, MultipleChoiceQuestionData } from '../types';
    ...
    try {
      ...
      if (!Array.isArray(triviaSet.question_data)) {
        ...
        for (let i = 0; i < triviaSet.question_data.length; i++) {
          const question = triviaSet.question_data[i] as MultipleChoiceQuestionData;
        ...
        const questionIds = triviaSet.question_data.map(
          (q) => (q as MultipleChoiceQuestionData).source_id,
        );
...
function validateQuestion(
  ...
  if (question.question_type !== 'multiple-choice') {
    errors.push(
      `Question ${index + 1}: question_type must be 'multiple-choice'`,
    );
  }

  if (!Array.isArray(question.wrong_answers)) {
    errors.push(`Question ${index + 1}: wrong_answers must be an array`);
  } else if (question.wrong_answers.length !== 3) {
    errors.push(
      `Question ${index + 1}: wrong_answers must contain exactly 3 items`,
    );
  } else {
    // Check for empty wrong answers
    for (let i = 0; i < question.wrong_answers.length; i++) {
      if (!question.wrong_answers[i] || question.wrong_answers[i].trim() === '') {
        errors.push(
          `Question ${index + 1}: wrong_answers[${i}] cannot be empty`,
        );
      }
    }
  }
