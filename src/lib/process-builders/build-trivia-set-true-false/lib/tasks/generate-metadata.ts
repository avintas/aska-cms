...
export const generateMetadataTask: ProcessBuilderTask = {
  ...
  async execute(context: TaskContext): Promise<TaskResult> {
    try {
      ...
      const selected = (previousResult.data as { selected: SourceTrueFalseQuestion[] })
        .selected;
...
function generateSlug(theme: string): string {
  if (!theme) {
    return `true-false-trivia-${Date.now()}`;
  }
...
function generateDescription(theme: string, questionCount: number): string {
  if (theme) {
    return `Test your knowledge with ${questionCount} ${theme} true/false questions.`;
  }
  return `Test your knowledge with ${questionCount} true/false trivia questions.`;
}
...
function determineCategory(
  questions: SourceTrueFalseQuestion[],
): string | undefined {
