 ...
 * Transforms selected questions to TrueFalseQuestionData format
 */

import type { ProcessBuilderTask, TaskContext, TaskResult } from '../../../core/types';
import type {
  SourceTrueFalseQuestion,
...
export const assembleDataTask: ProcessBuilderTask = {
  ...
  async execute(context: TaskContext): Promise<TaskResult> {
    try {
      ...
      const selected = (selectResult.data as { selected: SourceTrueFalseQuestion[] })
        .selected;
      ...
      // Note: Database uses is_true, but question_data JSONB uses correct_answer for consistency
      const questionData: TrueFalseQuestionData[] = selected.map(
        (question, index) => {
          return {
            ...
            correct_answer: question.is_true, // Map from database is_true to question_data correct_answer
