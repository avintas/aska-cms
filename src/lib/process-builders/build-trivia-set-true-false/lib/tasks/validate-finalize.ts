...
import type { TrueFalseTriviaSet, TrueFalseQuestionData } from '../types';
    ...
    try {
      ...
      if (!Array.isArray(triviaSet.question_data)) {
        ...
        for (let i = 0; i < triviaSet.question_data.length; i++) {
          const question = triviaSet.question_data[i] as TrueFalseQuestionData;
        ...
        const questionIds = triviaSet.question_data.map(
          (q) => (q as TrueFalseQuestionData).source_id,
        );
...
function validateQuestion(
  ...
  if (typeof question.correct_answer !== 'boolean') {
    errors.push(`Question ${index + 1}: correct_answer must be a boolean (true or false)`);
  }

  if (question.question_type !== 'true-false') {
    errors.push(
      `Question ${index + 1}: question_type must be 'true-false'`,
    );
  }
