 ...
 * Transforms selected questions to WhoAmIQuestionData format
 */
...
export const assembleDataTask: ProcessBuilderTask = {
  ...
  async execute(context: TaskContext): Promise<TaskResult> {
    try {
      ...
      const selected = (selectResult.data as { selected: SourceWhoAmIQuestion[] })
        .selected;
      ...
      const questionData: WhoAmIQuestionData[] = selected.map(
        (question, index) => {
          return {
