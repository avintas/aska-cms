...
export const generateMetadataTask: ProcessBuilderTask = {
  ...
  async execute(context: TaskContext): Promise<TaskResult> {
    try {
      ...
      const selected = (previousResult.data as { selected: SourceWhoAmIQuestion[] })
        .selected;
...
function generateSlug(theme: string): string {
  if (!theme) {
    return `who-am-i-trivia-${Date.now()}`;
  }
...
function generateDescription(theme: string, questionCount: number): string {
  if (theme) {
    return `Test your knowledge with ${questionCount} ${theme} Who Am I questions.`;
  }
  return `Test your knowledge with ${questionCount} Who Am I trivia questions.`;
}
...
function determineCategory(
  questions: SourceWhoAmIQuestion[],
): string | undefined {
